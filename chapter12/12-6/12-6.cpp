...
void search(vector<string>& v, string word) {// 벡터 v에서 word를 찾아 출력
	...
	{
		int index = v[i].find(word);	// v[i] 단어가 word 문자열을 포함하는지 검사. -1이 리턴되면 포함되지 않음
...
int main() {
	...
	fileRead(wordVector, fin);		//	파일 전체를 wordVector에 라인별로 넣기
	fin.close();
	...
	while (true)
	{
		...
		if (word == "exit") {// exit를 입력하면 프로그램 종료
			break;
		}		// 프로그램 종료
